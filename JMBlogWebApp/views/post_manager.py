...
from repository.repository_factory import RepositoryFactory as repository
...
def index():
    return render_template("list_posts.html",
                           database=repository.get_source().get_all())

@post_manager.route('/<uuid:current_index>')
def content(current_index):
    return render_template("view_post.html",
                           current=repository.get_source().get_by_id(current_index))
...
def add_item():
    if request.method == "POST":
        ...
        repository.get_source().add_post(to_add)
...
def remove_item(current_index):
    repository.get_source().remove(current_index)
    return redirect('/')
...
def update_item(current_index):
    ...
    if request.method == "POST":
        ...
        current.update(
            ...
            request.form['AuthorInput'],
            request.form['ContentInput'])
        repository.get_source().update_post(current)
